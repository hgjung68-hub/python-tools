...
# 1. 엔진 초기화
print("\n" + "="*60)
print("🚀 [CISS] Contract Integrated Search System 가동")
print("🚀 멀티 폴더 순차 처리 모드 활성화")
...
def process_single_base_path(current_base_path):
    """
    지정된 하나의 상위 폴더(예: Contract_1) 내의 하위 폴더들을 처리
    """
    if not os.path.exists(current_base_path):
        print(f"\n❌ 경로 없음: {current_base_path} 폴더를 건너띔니다.")
        return

    # 미처리 폴더 스캔 (Ok-, Err- 제외)
    folders = [f for f in os.listdir(current_base_path) 
               if os.path.isdir(os.path.join(current_base_path, f)) 
    ...
    for idx, folder_name in enumerate(folders):
        ...
        folder_full_path = os.path.join(current_base_path, folder_name)
        ...
        new_folder_path = os.path.join(current_base_path, f"{status_prefix}{contract_no}")
...
def run_all_ciss_batches(base_paths):
    start_time = time.time()
    for b_path in base_paths:
        print(f"\n{'#'*60}")
        print(f"🏁 대상 폴더 시작: {b_path}")
        print(f"{'#'*60}")
        process_single_base_path(b_path)
    
    end_duration = (time.time() - start_time) / 3600
    print(f"\n✨ 모든 배치 종료! (총 소요시간: {end_duration:.2f}시간)")

if __name__ == "__main__":
    # 처리하고 싶은 상위 폴더들을 리스트에 넣으세요
    target_base_paths = [
    ...
    run_all_ciss_batches(target_base_paths)